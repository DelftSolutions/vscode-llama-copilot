...
describe('wizard webview contract (media/onboarding + media/js)', () => {
	...
	describe('index.html (static markup)', () => {
		...
		it('contains the icon placeholders for every icon usage (light + dark mark, sidebar + done screen variants; replaced globally by wizardUI.ts)', () => {
			// 1 in comment + 1 sidebar + 3 done variants = 5 light, 5 dark
			const lightCount = html.match(new RegExp(ICON_TOKEN, 'g'))?.length ?? 0;
			const darkCount = html.match(new RegExp(ICON_TOKEN_DARK, 'g'))?.length ?? 0;
			expect(lightCount).toBeGreaterThanOrEqual(4);
			expect(darkCount).toBeGreaterThanOrEqual(4);
