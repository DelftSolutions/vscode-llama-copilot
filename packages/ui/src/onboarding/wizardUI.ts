...
export type WizardStep = 'mode' | 'downloading' | 'model' | 'starting' | 'remote' | 'done';
...
export interface WizardForward {
	label: string;
	localHost: string;
	localPort: number;
	remotePort: number;
...
export type DoneVariant = 'local' | 'reconnect' | 'manual';
...
export interface WizardState {
	...
	// Screen 4 — remote (SSH setup)
	isRemoteSession: boolean;
	sshHost: string | null;
	forwards: WizardForward[];
	sshSetupStatus: 'pending' | 'previewing' | 'applied' | 'skipped' | 'error';
	sshSetupError: WizardSetupError | null;
	sshDiffPreview: string | null;
