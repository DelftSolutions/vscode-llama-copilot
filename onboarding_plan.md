...
## TL;DR
Build a single-step-machine **WebviewPanel wizard** (mode choice → binary download → model pick → model start → done) hooked into extension activation and `server.managed` config changes. It reuses the existing webview pattern (`src/server/modelsIniUI.ts`), binary download (`BinaryManager`), device query (`queryDevices`), preset model (`MODEL_PRESETS`), and server lifecycle (`LlamaServerManager`). Onboarding state (step + done/skipped) persists in `context.globalState`; `llamaCopilot.server.managed` is written to **true as soon as the user picks the managed mode on screen 1** (re-written as a fallback when the managed path completes; reverted if the user then goes Back and picks Advanced/Skip). Upgrading users with a working setup never see the wizard.
...
7. **`src/extension.ts`**:
   ...
   - Extend existing `onDidChangeConfiguration` listener: `server.managed` false→true transition with no onboarding state → open wizard directly at the download/model step (managed path implied). No-ops when onboarding state already exists (in-progress wizard → it already drives setup; done/skipped → normal managed flow). The wizard's own screen-1 write of the flag triggers this path and is absorbed by the same guard.
...
9. **New `src/onboarding/onboarding.ts`**: `OnboardingOrchestrator` — the state machine driver:
   ...
   - **Advanced** → `executeCommand('llamaCopilot.openEndpointSettings')`, state `done`, `server.managed` left false (default) — reverted to false if this session set it from screen 1.
   - **Managed → downloading**: write `llamaCopilot.server.managed = true` (Global) the moment the user continues from screen 1 — unlocks the managed commands and next-launch auto-start even if setup is interrupted later — then `latestVersion()` → `downloadWithCallback` posting percent to panel + status bar; on failure post categorized error with Retry (re-invokes download); cancel keeps step `downloading`.
   ...
   - **Completion (state `running`)**: re-write `llamaCopilot.server.managed = true` as a fallback via `vscode.workspace.getConfiguration(CONFIG_SECTION).update('server.managed', true, ConfigurationTarget.Global)` (managed path only; normally already set on screen 1); state `done`; panel → done screen; `showInformationMessage('Your model is ready', 'Open Chat')` + Open Chat → `workbench.action.chat.open`; call shared `startServerAndAutoUpdate` tail (background update check); hide status bar (shown only during onboarding/loading/error, then hidden).
